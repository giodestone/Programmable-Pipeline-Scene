...
float4 main(InputType input) : SV_TARGET
{
    ...
    {
        color += saturate(sceneTex.Sample(sampler0, float2(input.tex.x + (onePx.x * ((float) i)), input.tex.y)) * weights[abs(i)]);
    }

    color.w = saturate(length(color.xyz) / 3.f); // Scale the alpha based on the color strenght (more needs to be bended in the more color there is).
